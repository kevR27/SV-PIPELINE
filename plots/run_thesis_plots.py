#!/usr/bin/env python3
"""Run all applicable thesis plots for one completed LRS sample.
...
def parse_args():
    p = argparse.ArgumentParser(description="Generate all available thesis plots for one LRS sample.")
    p.add_argument("--root", required=True, help="Pipeline output root corresponding to config_lrs.yaml path")
...
def main():
    ...
    jobs = [
        ...
        ([py, str(HERE / "plot_caller_concordance.py"), "--input", str(caller_summary), "--out-prefix", str(out / f"{s}_caller_concordance")], [caller_summary]),
