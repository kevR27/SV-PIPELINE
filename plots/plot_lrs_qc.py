#!/usr/bin/env python3
"""Plot LRS coverage QC from mosdepth summary/global distribution outputs."""
...
def parse_args():
    ...
    p.add_argument("--global-dist", required=True, help="*.mosdepth.global.dist.txt")
...
def load_summary(path: str) -> pd.DataFrame:
    ...
    if "chrom" not in lower or "mean" not in lower:
        raise ValueError(f"Unexpected mosdepth summary columns: {list(df.columns)}")
    return df
...
def main():
    ...
    chrom_col = next(c for c in summary.columns if c.lower() == "chrom")
    mean_col = next(c for c in summary.columns if c.lower() == "mean")
    summary[mean_col] = pd.to_numeric(summary[mean_col], errors="coerce")
    ...
    autosomal[[chrom_col, mean_col]].to_csv(prefix.with_name(prefix.name + "_chromosome_mean_depth.tsv"), sep="\t", index=False)
    ...
    if not autosomal.empty:
        autosomal["chrom_sort"] = autosomal[chrom_col].astype(str).str.replace("chr", "", regex=False).astype(int)
    ...
    if not genome_row.empty:
        ...
        if pd.notna(genome_mean):
            ax2.axhline(genome_mean, linestyle="--", linewidth=1.0, color="#D55E00", label=f"Genome mean = {genome_mean:.1f}×")
